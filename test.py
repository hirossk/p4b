...
while True:
    ...
    # 顔を認識し矩形にリスト化
    face_list = faceCascade.detectMultiScale(frame)

    if len(face_list) > 0:
        for face in face_list:
            frame = frame[face[1]:face[1] + face[3], face[0]:face[0] + face[2]] # トリミング 例：[68:218 , 55:205]
            frame = cv2.resize(frame, (28, 28)) # リサイズ
            image_gs = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) # グレースケール
        ...
        "Neural network definition1": "",
        }
        }
        body = str.encode(json.dumps(data))
        url = 'https://ussouthcentral.services.azureml.net/workspaces/52ec2e634276489cbaa4ab9ea9ee42cb/services/e9c13e606b5649c8a5eeb5d02b22b767/execute?api-version=2.0&details=true'
        api_key = 'CGkDuWsnlgXtNAqGiQnUsKhnIqifzgxdsZJRSXK07kZ8hHSfWspogpAEPZiSQ1dux9SjbDIP5nXSkjUv9sLDLA==' # Replace this with the API key for the web service
        headers = {'Content-Type': 'application/json', 'Authorization': ('Bearer ' + api_key)}
        #req = urllib.request.Request(url, body, headers)
        url = 'https://ussouthcentral.services.azureml.net/workspaces/52ec2e634276489cbaa4ab9ea9ee42cb/services/a1a73616b7ee4f7ca88294de855b5f5b/execute?api-version=2.0&details=true'
        api_key = 'o0W+Zv0PaQ2QyotovybqUveB4WDOomS+HqVrFdUmzi3/n35u86FLzOxTu48rTddx0bsh07Kv7vXnSwInupVlCA==' # Replace this with the API key for the web service
        headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key)}
        print(body)
        req = urllib.request.Request(url, body, headers) 
